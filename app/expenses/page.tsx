      ...
      <main className="flex-1 overflow-y-auto relative z-0 p-4 md:p-8 bg-slate-50 dark:bg-slate-950">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
           <div>
             <h2 className="text-2xl font-extrabold text-slate-800 flex items-center gap-2 dark:text-slate-100">
               <div className="p-2 bg-rose-100 rounded-lg text-rose-600 dark:bg-rose-900/30 dark:text-rose-400"><TrendingDown size={24}/></div>
               Minhas Despesas
             </h2>
             <p className="text-slate-500 text-sm mt-1 dark:text-slate-400">Gerencie todas as suas saídas.</p>
           </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
           {loading ? (
             ...
             <div className="p-12 text-center text-slate-400 dark:text-slate-500">Nenhuma despesa encontrada.</div>
           ) : (
                 ...
                 <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                   {transactions.map((t) => (
                     <tr key={t.id} className="hover:bg-slate-50 transition-colors group dark:hover:bg-slate-800/50">
                       ...
                       <td className="p-5 text-right font-bold text-rose-600 dark:text-rose-400">- {formatMoney(t.amount)}</td>
