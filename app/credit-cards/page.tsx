    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-8 relative z-0">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-extrabold text-slate-800">Meus Cartões</h2>
            <p className="text-slate-500">Gerencie seus limites e datas.</p>
          </div>
          {cards.length > 0 && (
            <button onClick={() => setIsModalOpen(true)} className="bg-brand-600 hover:bg-brand-700 text-white px-5 py-2.5 rounded-full flex items-center gap-2 font-semibold shadow-lg shadow-brand-600/30 transition-all hover:scale-105">
            ...
            {cards.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl shadow-xl shadow-slate-200/60 border-0">
                <div className="bg-brand-50 p-6 rounded-full mb-6">
                  <CreditCard className="text-brand-500 w-16 h-16" />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {cards.map(card => {
                  ...
                  return (
                    <div key={card.id} className="rounded-2xl shadow-xl shadow-slate-200/60 border-0 p-6 text-white relative overflow-hidden group h-56 flex flex-col justify-between transition-transform hover:-translate-y-2" style={{ backgroundColor: card.color || '#3b82f6' }}>
                      ...
                      <div className="flex justify-between items-start z-10">
                        ...
                        <CreditCard className="opacity-80" size={32} />
                      </div>
                      <div className="z-10 mt-4">
                        <div className="flex justify-between text-xs opacity-90 mb-2">
                          <div className="flex flex-col"><span className="opacity-70 text-[10px]">FECHAMENTO</span><span className="font-bold">DIA {card.closing_day}</span></div>
                          <div className="flex flex-col text-right"><span className="opacity-70 text-[10px]">VENCIMENTO</span><span className="font-bold">DIA {card.due_day}</span></div>
                        </div>
                        <div className="border-t border-white/20 pt-3 mt-1 flex justify-between items-end">
                          <div><p className="text-xs opacity-80">LIMITE TOTAL</p><p className="text-lg font-bold">{formatMoney(card.limit_amount)}</p></div>
                          <div className="text-right"><p className="text-xs opacity-80">USO</p><p className="text-sm font-bold">{usage.toFixed(0)}%</p></div>
                        </div>
                      </div>
                      <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => handleEdit(card)} className="bg-black/20 p-2 rounded-full hover:bg-white/20 transition-colors"><Pencil size={16} className="text-white" /></button>
                        <button onClick={() => handleDelete(card.id)} className="bg-black/20 p-2 rounded-full hover:bg-red-500/80 transition-colors"><Trash2 size={16} className="text-white" /></button>
                      </div>
                    </div>
                  );
                })}
