...
interface Asset { 
  id: number; name: string; ticker: string; type: 'FIXED' | 'FII' | 'STOCK' | 'CRYPTO'; quantity: number; current_amount: number; price?: number;
}
...
export default function InvestmentsPage() {
  ...
  const closeModals = () => {
    setIsAssetModalOpen(false); setAssetToEdit(undefined); setIsGoalModalOpen(false); setGoalToEdit(undefined);
    if (userId) fetchAll(userId);
  };
  ...
  const calculateRiskProfile = () => {
    ...
    if (riskPercentage > 40) return { label: "Arrojado", color: "text-rose-600", desc: "Maior exposição ao risco." };
    if (riskPercentage > 15) return { label: "Moderado", color: "text-amber-600", desc: "Equilíbrio." };
    return { label: "Conservador", color: "text-emerald-600", desc: "Segurança em primeiro lugar." };
  };
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-8 relative z-0">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-extrabold text-slate-800">Meus Investimentos</h2>
            <p className="text-slate-500 flex items-center gap-2 mt-1">
              Valores em tempo real. {userId && <button onClick={() => fetchAll(userId)} className="text-brand-600 hover:underline text-xs flex items-center gap-1 font-bold uppercase"><RefreshCw size={12}/> Atualizar</button>}
            </p>
          </div>
        </header>
            ...
            {activeTab === 'PORTFOLIO' && (
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/60 border-0 flex justify-between items-center relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                       <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Patrimônio</p>
                       <h3 className="text-3xl font-extrabold text-slate-900 mt-2">{formatMoney(totalInvested)}</h3>
                    </div>
                    <div className="bg-brand-100 p-3 rounded-2xl relative z-10"><Briefcase className="text-brand-600" size={32}/></div>
                  </div>

                  {/* PERFIL DE RISCO CARD */}
                  <div className="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/60 border-0 flex justify-between items-center relative overflow-hidden">
                     <div className="absolute top-0 right-0 -mr-12 -mt-12 w-32 h-32 bg-slate-50 rounded-full blur-3xl opacity-70"></div>
                    <div className="relative z-10">
                        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Perfil</p>
                        <h3 className={`text-3xl font-extrabold mt-2 ${riskProfile.color}`}>{riskProfile.label}</h3>
                        <p className="text-xs text-slate-400 mt-1 font-medium">{riskProfile.desc}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  {/* ALOCAÇÃO: DONUT CHART + LEGENDA LATERAL (Ajustado) */}
                  <div className="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/60 border-0 lg:col-span-1 h-auto min-h-[400px] flex flex-col">
                    ...
                    {/* Container Flexível para Chart e Legenda */}
                      ...
                      <div className="h-[200px] relative">
                         <ResponsiveContainer width="100%" height="100%">
                          <RePie>
                            ...
                            <Tooltip formatter={(v:number) => formatMoney(v)} contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)'}} />
                          </RePie>
                        </ResponsiveContainer>
                  ...
                  <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border-0 lg:col-span-2 flex flex-col">
                    <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                      <h4 className="font-bold text-lg text-slate-700">Seus Ativos</h4>
                      <button onClick={() => setIsAssetModalOpen(true)} className="text-sm bg-brand-600 text-white px-4 py-2 rounded-full hover:bg-brand-700 flex items-center gap-1 font-medium shadow-lg shadow-brand-600/30 transition-all"><Plus size={16} /> Adicionar</button>
                    </div>
                    <div className="overflow-auto flex-1">
                      <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50/50 text-slate-500 text-xs uppercase tracking-wider font-semibold">
                          <tr><th className="p-5">Ativo</th><th className="p-5">Qtd.</th><th className="p-5 text-right">Preço</th><th className="p-5 text-right">Total</th><th className="p-5"></th></tr>
                        </thead>
                          ...
                          {assets.map(asset => (
                            <tr key={asset.id} className="hover:bg-slate-50/80 transition-colors">
                              ...
                              <td className="p-5 text-right flex justify-end gap-2">
                                <button onClick={() => handleEditAsset(asset)} className="bg-slate-100 text-slate-400 hover:text-brand-600 p-2 rounded-lg transition-colors"><Pencil size={16} /></button>
                                <button onClick={() => handleDeleteAsset(asset.id)} className="bg-slate-100 text-slate-400 hover:text-red-600 p-2 rounded-lg transition-colors"><Trash2 size={16} /></button>
                              </td>
                            </tr>
                          ))}
                        </tbody>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {goals.map((goal) => {
                     ...
                     return (
                      <div key={goal.id} className="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/60 border-0 relative group transition-transform hover:-translate-y-1">
                        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                           <button onClick={() => handleEditGoal(goal)} className="bg-slate-100 p-2 rounded-full hover:bg-brand-100 hover:text-brand-600"><Pencil size={16} /></button>
                           <button onClick={() => handleDeleteGoal(goal.id)} className="bg-slate-100 p-2 rounded-full hover:bg-red-100 hover:text-red-600"><Trash2 size={16} /></button>
                        </div>
                        ...
                        <div className="flex justify-between items-end mb-3">
                          <span className="text-2xl font-extrabold text-purple-700">{formatMoney(goal.current_amount)}</span>
                          <span className="text-xs text-slate-400 mb-1 font-medium">de {formatMoney(goal.target_amount)}</span>
                        </div>
                        <div className="w-full bg-slate-100 rounded-full h-3 mb-2 overflow-hidden">
                          <div className="bg-purple-600 h-3 rounded-full transition-all duration-1000" style={{ width: `${percent}%` }}></div>
                        </div>
