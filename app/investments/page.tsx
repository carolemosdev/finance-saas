...
export default function InvestmentsPage() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      setUserId(user.id); setUserEmail(user.email); fetchAll(user.id);
    };
    checkUser();
  }, [router]);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/60 border-0 lg:col-span-1 h-auto min-h-[400px] flex flex-col">
                    ...
                    <div className="flex flex-col gap-6 flex-1">
                      <div className="h-[200px] relative"><ResponsiveContainer width="100%" height="100%"><RePie><Pie data={chartData} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value" stroke="none">{chartData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}</Pie><Tooltip formatter={(v:number) => formatMoney(v)} contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)'}} /></RePie></ResponsiveContainer><div className="absolute inset-0 flex items-center justify-center pointer-events-none"><Briefcase size={24} className="text-slate-300" /></div></div>
