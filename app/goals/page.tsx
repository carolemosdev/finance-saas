...
export default function GoalsPage() {
  ...
  const fetchData = async () => {
    ...
    // 2. Buscar Transações do ANO (Para o Gráfico)
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans flex-col md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto relative z-0 p-4 md:p-8">
        ...
        <div className="mb-8 flex justify-between items-center">
           <div>
             <h2 className="text-2xl font-extrabold text-slate-800 flex items-center gap-2">
                 <Target className="text-brand-600"/> Meus Planos
             </h2>
             <p className="text-slate-500 text-sm mt-1">Visão geral do seu progresso financeiro</p>
           </div>
        ...
        <div className="flex gap-3 mb-6">
           <button onClick={() => { setEditingGoal(undefined); setIsGoalModalOpen(true); }} className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
